...
use serenity_layer::prelude::{Args, Params};
...
pub async fn run_code(params: Params, code: String) -> Result<String, Error> {
    let mut errors = String::new();

    let warnings = params.warn.unwrap_or("false".to_owned());
    let channel = params.channel.unwrap_or("nightly".to_owned());
    let mode = params.mode.unwrap_or("debug".to_owned());
    let edition = params.edition.unwrap_or("2021".to_owned());
    ...
    let resp = reqwest::Client::new()
        ...
        .header(header::REFERER, "https://github.com/Seasons-of-Rust")
        .json(&request)
        .send()
        .await?;
    ...
    Ok(
        if result.len() + errors.len() > 1993 || lines > MAX_OUTPUT_LINES {
            format!(
                ...
                errors,
                get_playground_link(code, request).await?
            )
...
pub async fn get_playground_link(code: String, request: PlaygroundCode) -> Result<String, Error> {
    let mut payload = HashMap::new();
    payload.insert("code", code);

    // Make a reqwest post request from above
    let resp = reqwest::Client::new()
